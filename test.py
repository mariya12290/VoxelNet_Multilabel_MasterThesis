...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='campus_data')  #testing
    ...
    test_dir = os.path.join(cfg.DATA_DIR, 'campus_data')  #testing
            ...
            for batch in iterate_data(test_dir, shuffle=False, aug=False, is_testset=False, batch_size=args.single_batch_size * cfg.GPU_USE_COUNT, multi_gpu_sum=cfg.GPU_USE_COUNT):
                ...
                for i in range(len(results)):
                    for tag, result in zip(tags, results[i]):
                        os.makedirs(os.path.join(args.output_path, 'data',classes[i]), exist_ok=True)
                        of_path = os.path.join(args.output_path, 'data',classes[i], tag +'.txt')
                ...
                for i in range(len(results)):
                    if args.vis:
                        os.makedirs(os.path.join(args.output_path, 'vis',classes[i]), exist_ok=True)
                        for tag, front_image, bird_view, heatmap in zip(tags, front_images[i], bird_views[i], heatmaps[i]):
                            front_img_path = os.path.join( args.output_path, 'vis',classes[i], tag + '_front.jpg'  )
                            #bird_view_path = os.path.join( args.output_path, 'vis', tag +str(i)+ '_bv.jpg'  )
                            #heatmap_path = os.path.join( args.output_path, 'vis', tag +str(i)+ '_heatmap.jpg'  )
