...
parser.add_argument('-b', '--single-batch-size', type=int, nargs='?', default=2,
                    help='set batch size')
parser.add_argument('-l', '--lr', type=float, nargs='?', default=0.0001,
...
test_dir = os.path.join(cfg.DATA_DIR, 'testing')
        ...
        with tf.compat.v1.Session(config=config) as sess:
            ...
            for epoch in range(start_epoch, args.max_epoch):
                ...
                for batch in iterate_data(train_dir, shuffle=True, aug=True, is_testset=False, batch_size=args.single_batch_size * cfg.GPU_USE_COUNT, multi_gpu_sum=cfg.GPU_USE_COUNT):
                    ...
                    print('train: {} @ epoch:{}/{} loss: {:.4f} reg_loss: {:.4f} cls_loss: {:.4f} cls_pos_loss_p: {:.4f} cls_neg_loss_p: {:.4f} cls_pos_loss_c: {:.4f} cls_neg_loss_c: {:.4f} reg_loss_p: {:.4f} reg_loss_c: {:.4f} forward time: {:.4f} batch time: {:.4f}'.format(counter,epoch, args.max_epoch, ret[0], ret[1], ret[2], ret[3], ret[4], ret[5],ret[6],ret[7],ret[8], forward_time, batch_time))
                    with open('log/train.txt', 'a') as f:
                        f.write( 'train: {} @ epoch:{}/{} loss: {:.4f} reg_loss: {:.4f} cls_loss: {:.4f} cls_pos_loss_p: {:.4f} cls_neg_loss_p: {:.4f} cls_pos_loss_c: {:.4f} cls_neg_loss_c: {:.4f} reg_loss_p: {:.4f} ref_loss_c: {:.4f} forward time: {:.4f} batch time: {:.4f} \n'.format(counter, epoch, args.max_epoch, ret[0], ret[1], ret[2], ret[3], ret[4],ret[5],ret[6],ret[7],ret[8], forward_time, batch_time) )
                    ...
                    if counter % summary_val_interval == 0:
                        ...
                        batch = sample_test_data(test_dir, args.single_batch_size * cfg.GPU_USE_COUNT, multi_gpu_sum=cfg.GPU_USE_COUNT)
                        try:
                ...
                if ( epoch + 1 ) % 10 == 0:
                    ...
                    for batch in iterate_data(test_dir, shuffle=False, aug=False, is_testset=False, batch_size=args.single_batch_size * cfg.GPU_USE_COUNT, multi_gpu_sum=cfg.GPU_USE_COUNT):
                        ...
                        for i in range(len(results)):
                            for tag, result in zip(tags, results[i]):
                                os.makedirs(os.path.join(args.output_path, str(epoch), 'data',classes[i]), exist_ok=True)
                                of_path = os.path.join(args.output_path,str(epoch), 'data',classes[i], tag + '.txt')
                        ...
                        for i in range(len(results)):
                            if args.vis:
                                os.makedirs(os.path.join(args.output_path, str(epoch), 'vis',classes[i]), exist_ok=True)
                                for tag, front_image, bird_view, heatmap in zip(tags, front_images[i], bird_views[i], heatmaps[i]):
                                    front_img_path = os.path.join( args.output_path, str(epoch),'vis', classes[i], tag + '_front.jpg'  )
                                    bird_view_path = os.path.join( args.output_path, 'vis',classes[i], tag + '_bv.jpg'  )
                                    heatmap_path = os.path.join( args.output_path, 'vis', classes[i], tag + '_heatmap.jpg'  )
