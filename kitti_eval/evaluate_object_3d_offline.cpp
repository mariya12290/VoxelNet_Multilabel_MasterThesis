...
vector<int32_t> getEvalIndices(const string& result_dir) {
    ...
    if (dir) {
        while (entity = readdir(dir)) {
            ...
            if (len < 11) continue;
            int32_t index = atoi(path.substr(len - 11, 11).c_str());
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  // string gt_dir         = "data/object/label_1";
  // string result_dir     = "results/" + result_sha;
  std::string dir       = result_dir + "/plot";

  // create output directories
  system(("mkdir " + dir).c_str());

  std::string plot_dir1 = result_dir+"/plot" + "/Pedestrian";
  std::string plot_dir2 = result_dir+"/plot" + "/Cyclist";
  std::vector<std::string>plot_dir;
  plot_dir.reserve(2);
  plot_dir.push_back(plot_dir1);
  plot_dir.push_back(plot_dir2);

  system(("mkdir " + plot_dir1).c_str());
  system(("mkdir " + plot_dir2).c_str());


  // hold detections and ground truth in memory
  vector< vector<tGroundtruth> > groundtruth;
  std::vector<std::vector<std::vector<tDetection>>>   detections;
  detections.reserve(2);
  ...
  indices.reserve(2);
  std::vector<int32_t> index = getEvalIndices(result_dir + "/data/Pedestrian/");
  indices.push_back(index);
  indices.push_back(index);

  printf("number of files for Pedestrian evaluation: %d\n", (int)indices[0].size());
  printf("number of files for Cyclist evaluation: %d\n", (int)indices[1].size());
 std::vector<std::string> classes;
 classes.reserve(2);
 classes.push_back("Pedestrian/");
 classes.push_back("Cyclist/");

 //std::cout<<"indices size "<<indices.size()<<std::endl;
 for (auto j=0; j<indices.size(); ++j) {
  for(int32_t i =0;i<indices[j].size();++i ){
    // file name
    char file_name[256];

    sprintf(file_name,"%07d.txt",indices[j].at(i));
    ...
    vector<tDetection>   det  = loadDetections(result_dir + "/data/"+classes[j] + file_name,
            compute_aos, eval_image, eval_ground, eval_3d, det_success);
    detections[j].push_back(det);
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      ...
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, imageBoxOverlap, precision[0], aos[0], EASY, IMAGE)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, imageBoxOverlap, precision[1], aos[1], MODERATE, IMAGE)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, imageBoxOverlap, precision[2], aos[2], HARD, IMAGE)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir[j], CLASS_NAMES[c] + "_detection", CLASS_NAMES[c], precision, 0);
      printf("2D bounding boxes detection saved ... \n");
      if(compute_aos){
        printf("  start to compute aos ... \n");
        saveAndPlotPlots(plot_dir[j], CLASS_NAMES[c] + "_orientation", CLASS_NAMES[c], aos, 1);
        fclose(fp_ori);
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_ground[c]) {
      ...
      vector<double> precision[3], aos[3];
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, groundBoxOverlap, precision[0], aos[0], EASY, GROUND)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, groundBoxOverlap, precision[1], aos[1], MODERATE, GROUND)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, groundBoxOverlap, precision[2], aos[2], HARD, GROUND)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir[j], CLASS_NAMES[c] + "_detection_ground", CLASS_NAMES[c], precision, 0);
    }
  }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_3d[c]) {
      ...
      vector<double> precision[3], aos[3];
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, box3DOverlap, precision[0], aos[0], EASY, BOX3D)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, box3DOverlap, precision[1], aos[1], MODERATE, BOX3D)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections[j], compute_aos, box3DOverlap, precision[2], aos[2], HARD, BOX3D)) {
        mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
        return false;
      }
      fclose(fp_det);
      saveAndPlotPlots(plot_dir[j], CLASS_NAMES[c] + "_detection_3d", CLASS_NAMES[c], precision, 0);
    }
  }
  }
