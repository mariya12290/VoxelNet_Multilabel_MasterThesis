...
def aug_data(tag, object_dir):
    ...
    label = np.array([line for line in open(os.path.join(
        object_dir, 'label_1', tag + '.txt'), 'r').readlines()])  # (N')
    ...
    return newtag, rgb, lidar, voxel_dict, label 
    #https://github.com/steph1793/Voxelnet/blob/master/utils/aug_data.py  fix for disabling data augmentation
...
def worker(tag):
    ...
    with open(os.path.join(output_path, 'label_1', newtag + '.txt'), 'w+') as f:
        for line in label:
            f.write(line)
    print(newtag)
