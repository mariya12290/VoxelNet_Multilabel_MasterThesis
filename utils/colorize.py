...
def tf_colorize(value, factor=1, vmin=None, vmax=None, cmap=None):
    ...
    # normalize
    vmin = tf.compat.v1.reduce_min(value) if vmin is None else vmin
    vmax = tf.compat.v1.reduce_max(value) if vmax is None else vmax
    value = (value - vmin) / (vmax - vmin)  # vmin..vmax
    ...
    colors = tf.compat.v1.constant(cm.colors, dtype=tf.compat.v1.float32)
    value = tf.compat.v1.gather(colors, indices)
