...
class VFELayer(object):
    ...
    def apply(self, inputs, mask, training):
        ...
        # [K, T, units]
        repeated = tf.compat.v1.tile(aggregated, [1, cfg.VOXEL_POINT_COUNT, 1])

        # [K, T, 2 * units]
        concatenated = tf.compat.v1.concat([pointwise, repeated], axis=2)

        mask = tf.compat.v1.tile(mask, [1, 1, 2 * self.units])

        concatenated = tf.compat.v1.multiply(concatenated, tf.compat.v1.cast(mask, tf.compat.v1.float32))
...
class FeatureNet(object):
    ...
    def __init__(self, training, batch_size, name=''):
        ...
        self.feature = tf.compat.v1.placeholder(
            tf.compat.v1.float32, [None, cfg.VOXEL_POINT_COUNT, 7], name='feature')
        # [ΣK]
        self.number = tf.compat.v1.placeholder(tf.compat.v1.int64, [None], name='number')
        # [ΣK, 4], each row stores (batch, d, h, w)
        self.coordinate = tf.compat.v1.placeholder(
            tf.compat.v1.int64, [None, 4], name='coordinate')
        ...
        mask = tf.compat.v1.not_equal(tf.compat.v1.reduce_max(
